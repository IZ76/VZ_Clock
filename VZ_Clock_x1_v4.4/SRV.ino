void server_init(void) {
  ...
  server.on("/nmonOn", handle_nmon_on);
...
void handle_setup(){
  ...
  if(printCom){
    printTime();
    Serial.println("timeDay: "+String(timeDay)+", volBrightnessD: "+String(volBrightnessD)+", timeNight: "+String(timeNight)+", volBrightnessN: "+String(volBrightnessN)+",  kuOn: "+String(kuOn)+",  kuOff: "+String(kuOff)+", rotate: "+String(rotate0)+", clockNight: "+String(clockNight)+", buzzerSet: "+String(buzzerSet));
...
void handle_nmon_on(){
  if(verific()) return;
  if(server.arg("nmonOn")!="") setNMonFound = server.arg("nmonOn").toInt(); 
  if(printCom){
    printTime();
    Serial.println("Set setNMonFound= " + String(setNMonFound));
  }
  saveConfig(); 
  server.send(200, "text/plain", "OK");
}
